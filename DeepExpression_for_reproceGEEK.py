...
X_geek = np.load("../data/processed/"+args.s+"/geek_"+args.cl+".npy")
...
X_geek = X_geek[:,200-args.elen:200+args.elen]

X_exp = np.reshape(X_exp,(X_exp.shape[0],X_exp.shape[1],1))
X_geek = np.reshape(X_geek,(X_geek.shape[0],X_geek.shape[1],1))
...
###GEEK network
gmodel = Sequential()
gmodel.add(Dense(80, activation='relu',input_shape=(X_geek_train.shape[1],1)))
gmodel.add(Dropout(0.5))
gmodel.add(Flatten())
gmodel.add(Dense(40, activation='relu'))
...
model.add(Merge([model,gmodel], mode='concat'))
...
model.fit([X_exp_train,X_seq_train,X_geek_train], y_train,batch_size=args.batchsize, epochs=5000,shuffle=True, validation_data=([X_exp_val,X_seq_val,X_geek_val], y_val),callbacks=[checkpointer,earlystopper], verbose=2)

model.load_weights(name+"_combine_bestmodel.h5")
predicted = model.predict([X_exp_test,X_seq_test,X_geek_test])
